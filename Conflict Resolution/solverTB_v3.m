...
%initializations
r = UAV_Robot_v3(1,[1;1;1],zeros(2,2,2));                                   
...
%     end
    ...
    r(i) = UAV_Robot_v3(i, start_point, Wo, robot_priority(i));
...
while 1                                                                     %it will break with internal comprobations
    ...
    for i=1:num_robots
        ...
        if norm(r(i).destination-r(i).position) <= arrive_tolerance
            ...
            %Antiblocking System: if UAV hasn't reached destination and it has an incorrect path, recalculate path without temporary obstacles
            if (isempty(r(i).path) || size(r(i).path,1) ~= 3)
                r(i) = r(i).findPath;
            end
        end
    end
    
    %termination watchdog: if no robot has move (hovering is consider as movement), breaks simulation
    if move_probe == 0
        break;
    end
    ...
    for ii=1:num_robots                                                                       
        for jj=ii+1:num_robots
            ...
            if( distance < comms_distance)
                ...
                if arrived_robots(ii) == 0 && abs(randn * comm_error) < 1   %make 2nd UAV communicate with 1st IF the 1st hasn't reach destination (1st will listen and evaluate) AND if there's no error
                    r(jj).communicate(ii);
                end
                
                if arrived_robots(jj) == 0 && abs(randn * comm_error) < 1   %make 1st UAV communicate with 2nd IF the 2nd hasn't reach destination (2nd will listen and evaluate) AND if there's no error
                    r(ii).communicate(jj);
                end
