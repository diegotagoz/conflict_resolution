% Simulation script for UAV Collision Solver v3, which adds a FMredo improvement, hovering as another solving method, error in comms, termination watchdog (instead of timeout), antiblocking system and minimal distance as a new performance parameter. Path planning by Fast Marching method and Collision Solving by Velocity Modification (VelMod), hovering (Hover) and Fast Marching replanning (FMredo) methods. Uses the UAV class 'UAV_Robot_v3_stat'. Finish when all UAVs arrive their destination or when no UAV moves in a iteration. Intended to extract stadistical data, so it's optimized for computation time, without graphic results. It can't run itself, you need to run the script "stat_v2.m" and it will use this script.
tic;                                                                        %start counter
...
total_comms = 0;
succesful_comms = 0;
moving_iterations = zeros(1,num_robots);
hover_iterations = zeros(1,num_robots);
...
%     end
    ...
    r(i) = UAV_Robot_v3_stat(i, start_point, Wo, robot_priority(i));
...
while 1
    ...
    for i=1:num_robots
        ...
        %if UAV has arrived to destination
        ...
        %if UAV has not arrived to destination
        else
            if r(i).velocity == 0
                hover_iterations(i) = hover_iterations(i) + 1;              %if it's hovering, increment statistical parameter
            end
            moving_iterations(i) = moving_iterations(i) + 1;                %increment statistical parameter
            if (isempty(r(i).path) || size(r(i).path,1) ~= 3)               %AntiBlocking sistem: if path is incorrect, call path planning method
                r(i) = r(i).findPath;
            end
        end
    end
...
%                     disp(['REAL COLLISION DETECTED BETWEEN UAV ' num2str(ii) ' and UAV ' num2str(jj)]);
                ...
                if arrived_robots(ii) == 0                                  
                    if abs(randn * comm_error) < 1                          
                        r(jj).communicate(ii);                              %make 2nd UAV communicate with 1st IF the 1st hasn't reach destination (1st will listen and evaluate) AND if there's no error
                        succesful_comms = succesful_comms + 1;              %increment statistical parameter
                    end
                    total_comms = total_comms + 1;                          %increment statistical parameter
                end
                
                if arrived_robots(jj) == 0
                    if abs(randn * comm_error) < 1
                        r(ii).communicate(jj);                              %make 1st UAV communicate with 2nd IF the 2nd hasn't reach destination (2nd will listen and evaluate) AND if there's no error
                        succesful_comms = succesful_comms + 1;              %increment statistical parameter
                    end
                    total_comms = total_comms + 1;                          %increment statistical parameter
                end
