% Class/Object of UAV Robot version 2. Works with the simulation file 'solverTB_v2_stat'. Intended to extract stadistical data, so it's optimized por computation time: without command window messages
...
classdef UAV_Robot_v2_stat
    ...
    methods
        %constructor method
        function uav = UAV_Robot_v2_stat(ID, position, Wo, priority, level, saturation)
            if nargin > 3.
        ...
        function uav = receive(uav, mate)
            ...
            if detected == 1
                ...
                while detected == 1 && uav.velocity > 1 
                    ...
                    if ~isempty(uav.path)
                        detected = 0;
                        j = min(r(mate).velocity,size(r(mate).path,2)):min(r(mate).comprobation_length * r(mate).velocity,size(r(mate).path,2));
                        index = 1;
                        for i = min(uav.velocity,size(uav.path,2)):uav.velocity:min(uav.comprobation_length * uav.velocity,size(uav.path,2))         %min for preventing invalid indexes
                            if ~isempty(r(mate).path)
                                mate_probe = r(mate).path(:,j(index));
