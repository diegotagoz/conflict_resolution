% Class/Object of UAV Robot version 3. Works with the simulation file 'solverTB_v3_stat'. Intended to extract stadistical data, so it's optimized por computation time: without command window messages
...
% move(steps): Moves only if hasn't reached destination, 'steps' is optional velocity to be forced in, not recommended
...
classdef UAV_Robot_v3_stat
    ...
    properties
        ...
        Wr = [];                            %temporary map with UAVs as obstacles, resets in every iteration
        W=[];                               %double map, first potential           
        Wf = [];                            %first potential modified
        D = [];                             %double map, second potential        
        saturation = 0.75;                  %for FM potential map
        velocity = 4;                       %velocity of movements, default/maximum is 4 path elements in each calling
        %solving parameters
    ...
    methods
        ...
        %constructor method
        function uav = UAV_Robot_v3_stat(ID, position, Wo, priority, level, saturation)
            if nargin > 3
        ...
        function uav = move(uav,steps)
            ...
            global move_probe;                                              %global variable for termination watchdog
            ...
            if norm(uav.destination-uav.position) > uav.arrive_tolerance && size(uav.path,1)==3 && ~isempty(uav.path)   %only moves if UAV hasn't reached destination and the path is correct (to avoid index errors)
                ...
                %only if the UAV is not hovering
                if vel > 0
                ...
                move_probe = 1;                                             %notifies that UAV has succesfully moved
            end
            uav.Wr = uav.Wo;                                                %resets temporary obstacle map 
...
%COMMS
        ...
        function uav = receive(uav, mate)
            ...
            if norm(uav.position - r(mate).position) <= uav.collision_tolerance
                ...
                if ~isempty(uav.path)
                    if isempty(r(mate).path) || size(r(mate).path,1) ~= 3 || r(mate).velocity == 0      
                        mate_probe = r(mate).position;                      %if mate's path is incorrect or is hovering, I will only consider his actual position
                        %j is a vector with the mate's positions that I will compare to mines
                        %j for not moving mate
                        j = 1;
                    else
                        %j for moving mate
                        j = min(r(mate).velocity,size(r(mate).path,2)):r(mate).velocity:min(r(mate).comprobation_length * r(mate).velocity,size(r(mate).path,2));   %min for preventing invalid indexes
                    end
                    
                    %if I'm hovering, the loop moves through j
                    if uav.velocity == 0
                        for k = j
                            %compare my position with the mate's comparing positions
                            if norm(uav.position - r(mate).path(:,k)) <= uav.collision_tolerance
                                detected = 1;
                                break;
                            end
                        end
                    %if I'm not hovering   
                        ...
                        for i = min(uav.velocity,size(uav.path,2)):uav.velocity:min(uav.comprobation_length * uav.velocity,size(uav.path,2))         %min for preventing invalid indexes
                            ...
                            %positions (if it is moving) or with his actual
            ...
            if detected == 1
                ...
                %VelMod and Hover METHODS
                %this loop will try to solve it by reducing velocity until it is solved or it reaches the minimum velocity (0 - Hover)
                while detected == 1 && uav.velocity > 0 
                    uav.velocity = uav.velocity - 1;                        %decreases velocity
                    
                    %it runs again the conflict detector (only if the path is correct)
                    if ~isempty(uav.path) && size(uav.path,1) == 3
                        detected = 0;
                        if isempty(r(mate).path) || size(r(mate).path,1) ~= 3 || r(mate).velocity == 0
                        ...
                        if uav.velocity == 0
                            for k = j
                                if ~ (isempty(r(mate).path) || size(r(mate).path,1) ~= 3 || r(mate).velocity == 0)
                                    mate_probe = r(mate).path(:,k);
                                end
                                if norm(uav.position - mate_probe) <= uav.collision_tolerance
                                    detected = 1;
                                    break;
                                end
                            end
                         	...
                         	index = 1;
                            for i = min(uav.velocity,size(uav.path,2)):uav.velocity:min(uav.comprobation_length * uav.velocity,size(uav.path,2))         %min for preventing invalid indexes
                                if ~ (isempty(r(mate).path) || r(mate).velocity == 0)
                                    mate_probe = r(mate).path(:,j(index));
                                end
                                if norm(uav.path(:,i) - mate_probe) <= uav.collision_tolerance
                                    detected = 1;
                ...
                if detected == 0                                            %if the conflict is no longer detected, it has been resolved by VelMod or by Hover     
                    %disp(['Collision avoided by reducing UAV ' num2str(uav.ID) ' speed to ' num2str(uav.velocity)]);
                    velmod_reduction_accum = velmod_reduction_accum + uav.velocity; %modify statistical parameter         
                    if uav.velocity == 0
                        collisions_avoided_hover = collisions_avoided_hover + 1;    %increment statistical parameter           
                    else
                        collisions_avoided_velmod = collisions_avoided_velmod + 1;  %increment statistical parameter
                    end
                    uav.velocity_lock = 1;                                  %lock velocity for increasing
                    
                    %introduce mate position as obstacle in temporary map for posible FMredo solving in this same iteration. Max/min to avoid index errors.
                    uav.Wr(round(max(r(mate).position(1)-uav.collision_tolerance,1)):round(min(r(mate).position(1)+uav.collision_tolerance,size(uav.Wr,1))), ...                %min y max for preventing invalid indexes
                       round(max(r(mate).position(2)-uav.collision_tolerance,1)):round(min(r(mate).position(2)+uav.collision_tolerance,size(uav.Wr,2))), ...
                       round(max(r(mate).position(3)-uav.collision_tolerance,1)):round(min(r(mate).position(3)+uav.collision_tolerance,size(uav.Wr,3)))) = 1;
                    
                       
                   %introduce mate future position(s) as obstacle(s) in temporary map for posible FMredo solving in this same iteration. Max/min to avoid index errors.
                    if size(r(mate).path,1) == 3 && size(r(mate).path,2)>= (uav.future_pos * r(mate).velocity) 
                        for i = r(mate).velocity:r(mate).velocity:uav.future_pos * r(mate).velocity
                            uav.Wr(round(max(r(mate).path(1,i)-uav.collision_tolerance,1)):round(min(r(mate).path(1,i)+uav.collision_tolerance,size(uav.Wr,1))), ...
                               round(max(r(mate).path(2,i)-uav.collision_tolerance,1)):round(min(r(mate).path(2,i)+uav.collision_tolerance,size(uav.Wr,2))), ...
                               round(max(r(mate).path(3,i)-uav.collision_tolerance,1)):round(min(r(mate).path(3,i)+uav.collision_tolerance,size(uav.Wr,3)))) = 1;
                        end
                    end
                else
                    %disp(['Unable to avoid collision by reducing UAV ' num2str(uav.ID) ' speed.']);
                    uav.velocity = 3;                                       %resets velocity to 3
                    uav.velocity_lock = 0;                                  %unlocks velocity
                    
                    
                    %FMredo METHOD
                    %disp('Trying to avoid collision with FMredo');
                    %introduce mate position as obstacle in temporary map. Max/min to avoid index errors.
                    uav.Wr(round(max(r(mate).position(1)-uav.collision_tolerance,1)):round(min(r(mate).position(1)+uav.collision_tolerance,size(uav.Wr,1))), ...                %min y max for preventing invalid indexes
                       round(max(r(mate).position(2)-uav.collision_tolerance,1)):round(min(r(mate).position(2)+uav.collision_tolerance,size(uav.Wr,2))), ...
                       round(max(r(mate).position(3)-uav.collision_tolerance,1)):round(min(r(mate).position(3)+uav.collision_tolerance,size(uav.Wr,3)))) = 1;
                    
                       
                   %introduce mate future position(s) as obstacle(s) in temporary map. Max/min to avoid index errors.
                    if size(r(mate).path,1) == 3 && size(r(mate).path,2)>= (uav.future_pos * r(mate).velocity) 
                        for i = r(mate).velocity:r(mate).velocity:uav.future_pos * r(mate).velocity
                            uav.Wr(round(max(r(mate).path(1,i)-uav.collision_tolerance,1)):round(min(r(mate).path(1,i)+uav.collision_tolerance,size(uav.Wr,1))), ...
                               round(max(r(mate).path(2,i)-uav.collision_tolerance,1)):round(min(r(mate).path(2,i)+uav.collision_tolerance,size(uav.Wr,2))), ...
                    ...
                    Wnew = bwdist(uav.Wr);                                  %new distances map
                    sat = uav.saturation;
                    ...
                    %if this calculated path is correct, increment statistical parameter
                    if size(uav.path,1)==3 && ~isempty(uav.path)
                        collisions_avoided_fmredo = collisions_avoided_fmredo + 1;
                    end
                end
